...
x <- 650
rmote::rmote_device(height = 0.5*x, width = 2*x)
ggplot(uss_dfr, aes(x = days_treated, 
    ...
    geom_line(aes(group = patient_id, 
                  colour = Status)) + 
#    geom_smooth(colour = "#525252", se = FALSE) + 
...
x <- 150
rmote::rmote_device(height = 1*x, width = 10*x)
...
ggplot(hm_dfr, aes(x = Treatment, y = hallmark, fill = nes)) +
    ...
    labs(title = "Hallmark GSEA", 
         subtitle = "Enrichment is relative to\ndesensitised patients",
